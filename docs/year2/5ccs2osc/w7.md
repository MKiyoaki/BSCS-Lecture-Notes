...
1. Page Fault
  ...
  - Process
    ...
    - The OS can create **useful illusions** by handling page faults in creative ways. (Core idea)
      ...
      - Such idea can improve the effiency on handling page faults. 
...
2. The OS might do the following processes then resume the process. 
...
1. Definition
  ...
  - Another approach like lazy loading, in order to giving the illusion to run more processes or bigger processes.

    > The illusion is of the computer having more memory available to processes than it actually has physical memory.

  - By swapping data from and to disk with the physical memory (via the virtual memory space), with using page faults to trigger re-loading.
...
3. Swap Data Implementation
  ...
  - Two alternatives
    ...
    2. Demand-paged virtual memory - most of the ingredients of a modern virtual memory system.

##### 18.2. Demand-paged virtual memory
...
1. Introduction to frame allocation
...
2. Feature
  ...
  - Better to give *some* processes at least WSS frames. Others will have to wait!
    ...
    - Many other complementary strategies exist for managing out-of-memory conditions; e.g. Linux has ‘OOM killer’ (big hammer!)
