...
A **topological sorting** of a **Directed Acyclic Graph** $G = (V, E)$ is an ordered list of vertices $L = ⟨v1,v2,...,vn⟩$ such that
...
>1. Suppose, for contradiction, that there is a cycle $⟨C1, C2, . . . , Cn, C1⟩$: 
...
![undefined](https://upload.wikimedia.org/wikipedia/commons/thumb/d/d4/4x4_grid_spanning_tree.svg/1920px-4x4_grid_spanning_tree.svg.png)
...
The termination time for Kruskal’s Algorithm is $O(|E| \log |E|)$.
...
The termination time for Prims’s Algorithm is $O|E|+|V | \log |V |$.
