...
4. Most real OSes are 'multi-user', meaning they distinguish multiple *user identities*. 
...
1. Concurrency and sharing are the most important features of OSs. 
...
1. A software middle layer could run a program on the hardware, by 

  - interpreting it, or
  - by *translating* it (compiling or rewriting...)

2. OSs don’t do either of these. They actually run your program on the actual hardware, but still keep control. The process is as follows,
...
3. Any *interrupt* will put the CPU in kernel mode and jump into the OS
