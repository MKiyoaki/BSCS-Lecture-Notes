...
***Theorem 10.1.*** The Boolean Satisfiability problem **SAT** is **NP-complete**. 

***Proof.*** 

>We want to show that every problem $X \in NP$ can be reduced to **SAT**. 
>
>1. Let $X \in NP$ be **any problem** belonging to the class **NP**. By definition there is some NDTM $M$ such that, 
>
...
>4. We then have that 
>
>   $w \in X \iff F_{M,w}$ is satisifiable 
>
...
>	2. Else
>		3. $L \leftarrow$ `MERGESORT([a1,...,a[n//2]])`
>		3. $R \leftarrow$ `MERGESORT([a[n//2+1],...,an])`
>		3. $A\leftarrow$ `MERGE(L, R)`
>		3. Return $A$
...
>
>>$T(k+1) &=& 2T(\lceil\frac{k+1}{2}\rceil) + (k+1)\\&\geq& 2(\lceil\frac{k+1}{2}\rceil)\log_2(\lceil\frac{k+1}{2}\rceil)+(k+1)\\&\geq& 2(\frac{k+1}{2})\log_2(\frac{k+1}{2})+(k+1) \\&=&(k+1)\log_2(k+1)$
...
>
>>$F(k+1) &=& F(k) + F(k-1) \\&=& (\frac{\Phi^k-\phi^k}{\sqrt{5}}) + (\frac{\Phi^{k-1}-\phi^{k-1}}{\sqrt{5}})\\&=& (\frac{(\Phi+1)\Phi^{k-1}-(\phi+1)\phi^{k-1}}{\sqrt{5}})\\&=& \frac{\Phi^{k+1}-\phi^{k+1}}{\sqrt{5}}$
...
***Theorem 13.1.*** Let $T(n)$ be a monotonically increasing recurrence relation such that, 

$T(n)=aT(\frac{n}{b})+f(n)$ for some constants $a \geq 1, b \geq 2$. Then

$T(n)=\begin{cases} \Theta(n^k) &\text{if }f(n) \in \Omicron(n^{k-\epsilon})\\ \Theta(n^k\log_2n) &\text{if } f(n) \in \Theta(n^k)\\ \Theta(f(n)) &\text{if } f(n) \in \Omega(n^{k+\epsilon})\end{cases}$
