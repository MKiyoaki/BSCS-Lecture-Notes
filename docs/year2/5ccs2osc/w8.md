...
- I/O Steps
  ...
  1. programmed I/O & polling
     - Reading some data from disk (on x86). 
     - Meanwhile, since the disk is slow, we can just keep asking whether it’s ready. This is **polling**. 
  ...
  2. interrupts
     ...
     - Optimization
       ...
       - Direct memory access (DMA)
         - A direct memory access (DMA) controller allows a device to read/write data from memory by itself. 
