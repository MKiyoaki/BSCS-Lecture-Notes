...
>
> Small-Step Operational Semantics - defined by induction
>> Components
...
>
> Big-Step Operational Semantics - defined by evaluation
>
>> Deterministic
>> Big-Step Semantics
...
- Application of the Structural Induction Principle
  ...
  - Property
    ...
    > 1. Natural Numbers: 
    >    - Axiom: 0
    >    - Rule: $(\{n\}, n + 1)$, This is usually written in $\frac{n}{n+1}$​
    > 2. Evaluation relation for integer expressions in SIMP: 
    >    - Notation: 
    >      $(E,m) ⇓$ (where $n$ means that $E$ evaluates to $n$ in the state $m$)
    >    - Axioms:
...
- Definition
  ...
  - **Big-Step Operational Semantics** - a *transition system* for SIMP, where the <u>transition relation is defined by **evaluation**</u>.
    ...
    - That is, define by induction the binary relatio $⟨P, s⟩ \rightarrow^∗ ⟨P', s'⟩$ such that $⟨P', s'⟩$​ is *terminal*.
...
- Expressions
  ...
  \frac{⟨B_1,s⟩⇓⟨n_1,s'⟩ \ ⟨B_2,s'⟩⇓⟨n_2,s''⟩}{⟨E1 \text{ op } E_2,s⟩⇓⟨n,s''⟩, \text{if } n = n_1 \text{ op } n_2} &(\text{op}) \\
