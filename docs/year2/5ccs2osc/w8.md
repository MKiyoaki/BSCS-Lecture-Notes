...
>
>[➡️ WEEK IX - ](year2/5ccs2osc/w9.md)
>
>Outlines:
>
...
1. Definition
  - **Heap** and **Stack** are the two definitions of the memory management. The memory used by app is not a continuous memory space but in different memory areas, including
    ...
    - ...
  - **Heap**
    - When using allocator (e.g., alloc/new/copy...,etc. ) to create a new object, heap will allocate a memory segment. 
...
1. Memory allocation within user processes
  ...
  - `void* malloc(size_t size)`
    ...
    - A method that allocate *size* bits of uninitialized memory. 
...
1. `malloc` and system calls
  ...
  - Consider how `malloc` uses system calls
    ...
    - Another method is `mmap()`, as modern OSs provide more flexible ways to ask the OS for more memory, that don't care about the program break.
      - A `malloc()` implementation might do this and end up with multiple non-contiguous arenas. But we can extend the free list in our case. (A real `malloc()` won't use just a single free list)

2. Performance of a `malloc`
...
- Definition
  ...
  - **Stack**
    ...
    - When a function is invoked (e.g., the initial method of an app, `main()`, a *stack frame* will be puted into the stack, including the related information of this method, like parameters, return address, local variable, etc. When another function is invoked, another stack frame will be puted as into the stack. 
...
2. The Structure of (Structured) File
...
1. Processes are inherently communicative
...
3. Introdcution to File Descriptors (FD)
