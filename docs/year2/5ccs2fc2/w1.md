...
> Note: Some definitions and terminologies for finite automatas
...
> ref: 
>
> [Non-regular Languages from Medium](https://medium.com/@yashgaherwar2002/non-regular-languages-904b0c42e06b)
>
> [Slides from Stanford](https://web.stanford.edu/class/archive/cs/cs103/cs103.1142/lectures/16/Small16.pdf)
>
...
***Example 2.1.***   Prove that $L=\{a^nb^n|n\in N\}$ is non regular language. 
...
***Theorem 3.1.*** Considering a Turing Machine $M = <\Sigma, Q, q_{init}, q_{accpet}, q_{reject}, \delta>$,
  ...
  - Component
    ...
    > i.e., $\delta(current\ state, read\ symbol)\to (new\ state,\ write,\ move)$
...
- Definition: 
  ...
  - We used the following notation as a **shorthand** to describe the behaviour of $M$ on input word $w$: 
    ...
    $M(w):=\begin{cases}1&{if\ M\ accepts\ w}\\0&{if\ M\ rejects\ w}\\↑&{if\ M\ does\ not\ halt\ on\ w}\end{cases}$
...
- Terminology
  ...
  - The language decided by $L$ is the set: 
    ...
    $Language(M)=\{w \in \Sigma^*|M(w)\ accepts\ w\}$
---
...
- Component
  ...
  - Transition function $\delta:(Q \times \Sigma \bigcup \{⊔\}) \to P(Q \times \Sigma \bigcup \{⊔\} \times \{←, →\})$
    ...
    > i.e. $\delta(current\ state, read\ symbol) \to \{possible\ instructions\ (state,write,move)\} $
...
> ref: 
>
> [Stanford Encyclopedia of Philosophy](https://plato.stanford.edu/entries/church-turing/)
