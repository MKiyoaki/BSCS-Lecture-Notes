...
>12. Hierachical Clustering Algorithm
...
- Let $p_1, ... , p_n ∈ \mathbb{R}^D$ be a set of points in $D$ dimensions. Consider a metric for distance between points (such as Manhattan

  or Euclidean). Clusters at time $t$ are called $C^{(t)}_1,C^{(t)}_2,...,$. 
